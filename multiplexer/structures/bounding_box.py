...
class BoxList(object):
    ...
    def set_size(self, size):
        self.size = size
        bbox = BoxList(self.bbox, size, mode=self.mode, use_char_ann=self.use_char_ann)
    ...
    def add_field(self, field, field_data, check_consistency=True):
        self.extra_fields[field] = field_data
        if check_consistency:
            if field == "masks":
                assert len(field_data.polygons) == len(
                    self.bbox
                ), f"polygons = {field_data.polygons}, boxes = {self.bbox}"
            elif field == "rotated_boxes_5d":
                assert len(field_data.tensor) == len(
                    self.bbox
                ), f"rotated_boxes_5d = {field_data.tensor}, boxes = {self.bbox}"
