...
class BinaryLanguageGrouper(nn.Module):
    ...
    def forward(self, word_lang_probs):
        ...
        losses = {}
        losses["loss_group_1"] = self.loss_weight * F.relu(1.0 - torch.sum(lang_head_probs[:, 0]))
        losses["loss_group_2"] = self.loss_weight * F.relu(1.0 - torch.sum(lang_head_probs[:, 1]))
