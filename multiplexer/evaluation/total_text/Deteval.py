...
from .polygon_fast import area, area_of_intersection, iod
...
def total_text_eval_det(
    pred_dir=f"/checkpoint/{getpass.getuser()}/datasets/total_text/toy_eval/pred/",
    gt_dir=f"/checkpoint/{getpass.getuser()}/datasets/total_text/old_gt/Groundtruth/Polygon/Test",
    tp=0.6,
    tr=0.7,
):
    """
    Prediction format: y0,x0, ..... yn,xn. Each detection is separated by the
    end of line token ('\n')
    """
    input_dir = pred_dir
    ...
    def tau_calculation(det_x, det_y, gt_x, gt_y):
        ...
        try:
            ...
            print(
                "Division by zero: det_x = {}, det_y = {}, area(det_x, det_y) = {}".format(
                    det_x, det_y, area(det_x, det_y)
                )
            )
            return 0
    ...
    for input_id in allInputs:
        if input_id[-3:] != "txt":
            print("[Debug] skipped input_id {}".format(input_id))
            continue
    ...
    for idx in range(len(global_sigma)):
        ...
        print(
            "[{}] Precision: {} / Recall: {}".format(allInputs[idx], local_precision, local_recall)
        )
    try:
    ...
    print(
        "Precision: {:.2f} / Recall: {:.2f} / F-Score: {:.2f}".format(
