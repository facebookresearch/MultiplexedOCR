...
def get_tl_line_values(
    ...
    if LTRB:
        ...
        parts = line.split(",")
        numPoints = int((len(parts) - 1) / 2)
        points = [float(parts[i]) for i in range(2 * numPoints)]
...
def get_tl_line_values_from_file_contents(
    ...
    if double_line:
        for (line1, line2) in zip(lines[0::2], lines[1::2]):
            line1 = line1.strip()
            line1 += ","  # add dummy transcription

            points, confidence, _ = get_tl_line_values(
                line1, LTRB, withTranscription, withConfidence, imWidth, imHeight
            )
            transcription = line2.strip()
    ...
    if withConfidence and len(confidencesList) > 0 and sort_by_confidences:
        confidencesList, pointsList, transcriptionsList = (
            list(t)
            for t in zip(
                *sorted(zip(confidencesList, pointsList, transcriptionsList), reverse=True)
            )
        )
...
def main_evaluation(
    ...
    if show_result:
        eval = resDict["method"]
        print("===Total Text E2E Summary===")
        print(eval)
        print(
            "[E2E] Precision: {:.2f} / Recall: {:.2f} / H-mean: {:.2f}".format(
                eval["precision"] * 100, eval["recall"] * 100, eval["hmean"] * 100
            )
        )
        print("====================")
