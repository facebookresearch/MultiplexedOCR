...
# Date: 2017-04-18
# Description:
# File with useful functions to use by the evaluation scripts in the RRC website.
...
def validate_lines_in_file(
    ...
    calling the Line validation function for each line
    """
...
def validate_tl_line(
    ...
    Validate the format of the line.
        ...
        If maxWidth and maxHeight are specified, all points
            must be inside the imgage bounds.
    Posible values are:
...
def get_tl_line_values_from_file_contents(
    ...
    """
    Returns all points, confindences and transcriptions
        of a file in lists. Valid line formats:
...
def main_evaluation(
    ...
    """
    This process validates a method, evaluates it and if it succeeds, generates
    ...
    validate_data_fn: points to a method that validates the
        corrct format of the submission
    ...
    validate_data_fn(p["g"], p["s"], evalParams, languages=languages)
    evalData = evaluate_method_fn(p["g"], p["s"], evalParams, languages=languages)
    ...
    if show_result:
        print_eval_result(eval_result=resDict, languages=languages)

    return resDict


def print_eval_result(eval_result, languages=None):
    eval = eval_result["method"]
    language_info = "" if languages is None else (" (" + ":".join(languages) + ")")
    print(f"~~Task 1 Summary{language_info}~~")
    print(eval)
    print("H-mean{}: {:.4f}".format(language_info, eval["hmean"]))
    print("Precision{}: {:.4f}".format(language_info, eval["precision"]))
    print("Recall{}: {:.4f}".format(language_info, eval["recall"]))
    print("AP{}: {:.4f}".format(language_info, eval["AP"]))
    print("~~~~~~~~~~~~~~~~~~~~")
