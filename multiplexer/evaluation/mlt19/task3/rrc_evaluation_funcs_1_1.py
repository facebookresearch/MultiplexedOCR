...
def get_tl_dict_values(
    ...
    for i in range(num_points):
        ...
        if (
            ...
            raise Exception(
                "Incorrect format. Point #" + str(i + 1) + " childs have to be Integers)"
            )
...
def main_evaluation(
    ...
    """
    This process validates a method, evaluates it and
    if it succed generates a ZIP file with a JSON entry for each sample.
    Params:
    p: Dictionary of parmeters with the GT/submission locations.
        If None is passed, the parameters send by the system are used.
    default_evaluation_params_fn:
        points to a function that returns a dictionary with the default
        parameters used for the evaluation
    validate_data_fn:
        points to a method that validates the corrct format of the submission
    evaluate_method_fn:
        points to a function that evaluated the submission and return a
...
def print_eval_result(eval_result, languages=None):
    ...
    print(f"~~Task 3 Summary{language_info}~~")
    print(eval)
