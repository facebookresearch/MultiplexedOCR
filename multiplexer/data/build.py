...
from multiplexer.data import datasets as D
from multiplexer.data import samplers
from multiplexer.data.collate_batch import BatchCollator, BBoxAugCollator
...
def build_dataset(cfg, dataset_list, transforms, dataset_catalog, is_train=True):
    ...
    # for training, mix/concatenate all datasets into a single one
    if len(datasets) > 1:
...
def make_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0):
    ...
    if is_train:
        ...
        assert images_per_batch % num_gpus == 0, (
            "TEST.IMS_PER_BATCH ({}) must be divisible by the number " "of GPUs ({}) used."
        ).format(images_per_batch, num_gpus)
        images_per_gpu = images_per_batch // num_gpus
    ...
    if images_per_gpu > 1:
        ...
        logger.warning(
            ...
            "https://github.com/facebookresearch/Detectron/blob/master/configs/"
            "getting_started/tutorial_1gpu_e2e_faster_rcnn_R-50-FPN.yaml#L14"
        )
    ...
    # and we will apply transforms later
    transforms = (
        None if not is_train and cfg.TEST.BBOX_AUG.ENABLED else build_transforms(cfg, is_train)
    )
    ...
    for dataset in datasets:
        ...
        # If bbox aug is enabled, collator won't convert images to batched images
        collator = (
            BBoxAugCollator()
